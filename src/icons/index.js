...
function processIcon (currentSize, icon, buffer, mimeType, publicPath, shouldFingerprint) {
  ...
  const fileName = shouldFingerprint === true || shouldFingerprint === 'icons' ? `icon_${dimensions}.${generateFingerprint(buffer)}.${mime.getExtension(mimeType)}` : `icon_${dimensions}.${mime.getExtension(mimeType)}`
